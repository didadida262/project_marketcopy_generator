...
if %errorlevel% equ 0 (
    ...
    if %errorlevel% equ 0 (
        echo [成功] Llama 3.1 8B模型已安�?
        set AI_MODE=LLAMA
    ) else (
        echo [警告] Llama模型未安装，将使用备用模�?
        set AI_MODE=FALLBACK
    )
) else (
    echo [警告] Ollama服务未启动，将使用备用模�?
    set AI_MODE=FALLBACK
)
...
if exist ".env" (
    ...
    if %errorlevel% equ 0 (
        echo [成功] OpenAI API已配�?
        if "%AI_MODE%"=="FALLBACK" set AI_MODE=OPENAI
    ) else (
        echo [警告] OpenAI API未配�?
    )
) else (
    echo [警告] 环境配置文件不存�?
)
...
echo.
echo 启动后端服务...
start "AI营销文案生成�?- 后端服务" cmd /k "echo 启动后端服务�?.. && echo AI模式: %AI_MODE% && npm run server"
...
echo 相关文档:
echo   - README.md - 项目总览
echo   - QUICK_START_FINAL.md - 快速开�?
echo   - DEMO_GUIDE.md - 演示指南
echo   - PROJECT_STATUS.md - 项目状�?
echo.
